...
This chapter is organized as follows: section~\ref{prelim-cal}
provides a brief and general introduction to EM calorimeters, section~\ref{lyso-cal} is
dedicated to the timing properties of LYSO crystal scintillator based
calorimeters, section~\ref{mcp-cal} discusses the possibility of a precision timing sampling
calorimeter where the sensitive medium is a microchannel
plate~\cite[MCP], section~\ref{sil-cal} discusses the timing capabilities of a sampling
calorimeter where the sensitive medium is a 300 microns silicon
sensor. finally, section~\ref{cal-sum} summarizes the results and
...
bremsstralung (pair production). This process evolves until the newly
created particles have low energies and therefore other processes such as
photoelectric effect, Compton scatterring, and ionization start to
become relevant. Shower development is described independently of the
calorimeter material by two quantities: the \textbf{radiation length}
($X_{0}$) and the \textbf{Moli\`ere radius} ($R_{M}$). 

The radiation length is the characteristic amount traversed by high energy
...
The Moli\`ere radius is the characteristic size in the transverse
direction, ......
...
\paragraph{Wavelength shifting fibers readout (WLS Y11 \& DSB1)}
...
\clearpage
\section{Pixel MCP-PMT Timing}\label{mcp-cal}
