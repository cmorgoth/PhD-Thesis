...
contain the energy deposited by electrons, positrons, and photons (e/$\gamma$);
...
Homogenous EM calorimeters are usually made out of dense scintillating
crystals (BGO, BaF$_{2}$, LYSO, PbWO$_{4}$, among others) or liquid nobel gases
(Ar, Kr, Xe) and they provide an outstanding energy resolution for
e/$\gamma$. For example, the CMS EM calorimeter~\cite{cmsECAL} is
composed of PbWO$_{4}$ scintillating crystals coupled to avalanche
...
trigger systems, and the measurement of the missing transverse energy in
each proton-proton collision. Therefore, maintaining the current performance of the EM
...
chapter; this section is intended to provide a short overview of the
...
it is responsible for the shower development, while
...
distinction between gas and liquid or solid media is made, since there are
...
bremsstrahlung (pair production). The number of particles produced by
...
photoelectric effect, Compton scattering, and ionization start to
...
histogram shows the fractional energy per radiation length, and the
...
e/$\gamma$s in the longitudinal direction (along the original
...
In this chapter another precision timing calorimetric prototype
...
independent pixels preserves a good time resolution. The measured time resolution improves with the increase in the number of pixels used as 
...
charge distribution. Since the electron beam profile and purity varies at different beam energies, we collected between 10 and 50 thousand events for each beam energy, in order to ensure sufficiently large data samples. A fairly linear dependence between the measured
...
TOF distribution, i.e. $\Delta t = t_0-t_1$. The later exhibits a systematic dependence on the total charge measured in the silicon detector, as shown on the
...
\textit{time correction}. The correction is obtained by fitting a second degree
...
prototype calorimetric system is sufficiently low to extract signals from MIPs. Comparing the
...
the sensitive medium in sampling calorimeters, as is planned for example for the CMS Phase 2 
...
Recent advances in silicon sensors in terms of radiation tolerance, highly granular designs~\cite{Adloff:2011ha}, and cost per unit area
offers the possibility of their use as the sensitive media in a sampling
...
be use to reduce the detrimental effects of the high pileup environment
...
was not electromagnetically shielded and therefore some electronic
...
linear fit (solid red line) with the horizontal line corresponding to
...
examples of transverse shower profile measured by the HGC layer,
...
activity is concentrated in the central and the 6 neighboring pixels
-- these 7 pixel are more clearly represented in the left panel of
...
the tungsten absorber and the HGC layer, where it is observed that the
...
transverse shower spread, to achieve this we use the ratio of the
charge in the central pixel and the total charge in the 7 pixels as
proxy to the transverse shower profile:
...
layer was 1~mm. The results are summarized in
...
to better constrain the floating parameters. Nevertheless, the best time
...
Chapter~\ref{mcp-cal} the Photonics MCP-PMT located downstream of the HGC
...
calorimeter uses a front-end electronics readout based on the
...
50~ps smearing was applied. The results are summarized in
...
central pixel containing most of the energy of the shower. As expected,
the total
...
improve the timing performance, provided large signal to noise ratios in
...
45~ps and 30~ps, respectively. These studies suggest that the desirable
